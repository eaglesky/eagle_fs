...
int print_i_hd(FILE *file)
{
	int i;
	for( i = 0; i < INODE_LST_SZ*INODE_NUM_PER_BLK; i ++) {
	inode_incore * incore = iget(i);
	file_type tmp = incore->inode.itype;
	printf("File type field:\t%d\tlink:\t%d\ti:\t%d\t", tmp, incore->inode.link_num, incore->inode_num);
	if(tmp)
		printf("Error!\n");
	else
		printf("\n");
